...
const WFH = () => {
  ...
  const getLocation = () => {
    if (!navigator.geolocation) {
      ...
      navigator.geolocation.getCurrentPosition((position) => {
        ...
        setLat(null);
        setLng(null);
        setStatus('Tidak dapat mengambil lokasi anda. <br/> Lanjutkan absen tanpa Lokasi');
      });
    }
  }
  ...
  const sampleAbsent = e => {
    if (lat != null && lng != null) {
      ...
      formData.append('status_presence', 2);
              ...
              {selectedFile ?
                <>
                  ...
                  <div className="btn-upload" style={{ marginTop: '1vh' }}>
                    <div>
                      <input type='file' onChange={onSelectFile} id="img" style={{ display: 'none' }} accept="image/png, image/jpeg, image/jpg" />
                      <label htmlFor="img" className="btn btn-success">{t('btn_absen_masuk')}</label>
                    </div>
                    <div>
                      <input type='file' onChange={onSelectFile} id="img" style={{ display: 'none' }} accept="image/png, image/jpeg, image/jpg" />
                      <label htmlFor="img" className="btn btn-danger">{t('btn_absen_keluar')}</label>
                    </div>
                  </div>
                ...
                >
                  {listNews.map((i, index) => (
                    <div className="child-img-news" key={index}>
                      <img src={i.url} alt="curousel news" />
                      <p>{i.title}</p>
                    </div>
                  ))}
                </Carousel>
